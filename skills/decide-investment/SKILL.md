...
1. 使用 `$manage-investment-lifecycle` 和 `context_current` 读取生效的 Investor、Mandate 与 Attention Policy；使用 `portfolio_state_as_of` 从确认流水重建组合。Markdown 仅作可读材料，不作为精确持仓事实源。
...
7. 保持事实边界：Decision 发布时冻结 Context、Portfolio Calculation 和 Thesis Revision；只有用户明确确认的 Execution 才能产生 Ledger Entry。
