...
5. 冻结来源：对每个材料性来源调用 `investment_evidence_update(operation="publish_source")`。必须按内容声明 `evidence_type`：可观察事实 `observed_fact`、发行人/监管/交易所原始披露 `official_disclosure`、已验证分析 `validated_analysis`、预测信号 `predictive_signal`。不得把预测包装成事实；未知类型不能支持受限行动。`source_group` 必须表示真实独立来源组，不得为了通过验证而伪造独立性。价格、汇率和其他决策时点观测使用 `operation="market_snapshot"`。
